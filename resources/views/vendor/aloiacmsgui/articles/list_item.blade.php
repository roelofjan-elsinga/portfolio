<div class="flex mb-4 text-theme-darkest no-underline">
    ...
    <div class="w-3/4 px-4">
        ...
        <div class="text-sm text-gray-600">
            Tags ({{count($article->tags ?? [])}}): {{implode(', ', $article->tags ?? [])}}
        </div>

        @if(!is_null($article->getUpdateDate()) && $article->getUpdateDate()->diffInDays(now()) > 90)
            <strong class="text-red-500">Content was last updated {{$article->getUpdateDate()->diffInDays(now())}} days ago!</strong>
        @endif

        @if(is_null($article->getUpdateDate()))
            <strong class="text-red-500">Content has never been updated!</strong>
        @endif
